...
작업이 제너레이터를 반환하면(본문에 `yield` 가 있으면) 워커는 **첫 `yield` 값을
곧바로 결과 큐에 넣고**, 나머지는 같은 스레드에서 끝까지 돌린다. 오래 걸리는
후처리가 있지만 응답은 먼저 돌려줘야 할 때 쓴다.
...
class MyTask:
    ...
    def __call__(self, params):
        answer = quick_part(params)
        yield answer                  # 이 값이 결과 큐로 간다
        slow_part(params, answer)     # 응답 뒤에 계속 실행된다
```

규약:

- 결과는 **첫 `yield` 값 하나**다. 두 번째 이후 `yield` 값과 `return` 값은 버린다.
- `yield` 없이 `return` 만 하면 그 반환값이 결과다 (일반 작업과 같다).
- 첫 `yield` 전에 예외가 나면 일반 작업과 같이 `(task, TaskExecutionError)` 가 간다.
- **첫 `yield` 뒤에 예외가 나면 결과 큐에는 아무것도 추가로 가지 않는다.** 부모는
  이미 받은 값이 최종 결과라고 보게 되므로, 후처리의 성공/실패 상태는 작업 쪽에서
  직접 기록해야 한다. 워커는 traceback을 로그로만 남긴다.
- 부모 입장에서 조기 응답과 일반 결과는 같은 `(task, 값)` 모양이라 구분되지 않는다.
- 순차 모드에서는 나머지 실행이 끝나야 다음 작업을 꺼낸다. 응답만 빨라지고 워커
  처리량은 같다. 동시 모드(`max_workers > 1`)에서는 그 스레드가 끝날 때까지 점유된다.
- `stop()` 은 나머지 실행까지 끝난 뒤에 종료된다.
